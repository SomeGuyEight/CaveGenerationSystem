...
namespace Tessera.CaveGeneration {
    ...
    public class OnClickActionAttribute : Attribute {
       ...
       public OnClickActionAttribute(string actionString) {
          ...
          /// <summary>
       /// Inspector Editor/buttons below
       /// </summary>
       /// 
 #if UNITY_EDITOR
       [OnInspectorGUI]
       ...
       public class AsyncGenerationTask {
    ...
    *        private IEnumerable<CoreChunk> GetReadyChunks() {
            foreach (var coreChunk in GetWatchedChunks()) {
                ...
                if (coreChunk.Status != CoreChunkStatus.Pending && coreChunk.Status != CoreChunkStatus.Memoized)
                    continue;

                // Two adjacent chunks cannot be generating at the same time.
                if (_parallelism > 1) {
                    if (GetNeighbours(coreChunk.Cell).Any(c => c.status == CoreChunkStatus.Generating)) {
                        continue;
                    }
                }
